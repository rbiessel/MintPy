...
def read_vrt_info(vrt_file):
    ...
    xmin = 0
    ymin = 0
    xsize = int(root.get('rasterXSize'))
    ysize = int(root.get('rasterYSize'))
    xmax = xmin + xsize
    ymax = ymin + ysize
...
def prepare_metadata(meta_file, geom_src_dir, box=None):
    ...
    # add LENGTH / WIDTH
    atr = readfile.read_attribute(os.path.join(
        geom_src_dir, 'lat{}'.format(geom_ext)))
...
def prepare_timeseries(outfile, unw_file, metadata, processor, baseline_dir=None):
    ...
    with h5py.File(outfile, "a") as f:
        ...
        for i in range(num_file):
            ...
            if i > 1:
                f["timeseries"][i+1] += f["timeseries"][i]
...
def prepare_temporal_coherence(outfile, coh_file, metadata):
    ...
    # read data using gdal
    ds = gdal.Open(coh_file + '.vrt', gdal.GA_ReadOnly)
...
def main(iargs=None):
    ...
    box = read_vrt_info(
        os.path.join(inps.geomDir, 'lat.rdr.full.vrt'))

    geom_src_dir = inps.geomDir
    # metadata
